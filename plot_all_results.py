...
def plot_scaling(summary_path):
    ...
    fig.suptitle("Scalability Experiment (MNIST)", fontsize=15)
    fig.tight_layout()
...
def plot_ablation(summary_path):
    ...
    ax1.set_title("Final accuracy per mode")
    ax1.set_ylabel("Final accuracy")
    ax1.set_xticks(x)
    ax1.set_xticklabels(modes)

    ax2.bar(x, times, color="#e9c46a")
    ax2.set_title("Mean time per round")
    ax2.set_ylabel("Time (s)")
    ax2.set_xticks(x)
    ax2.set_xticklabels(modes)

    fig.suptitle("Ablation Experiment", fontsize=15)
    fig.tight_layout()
