...
def approve_client_update(job_addr: str, update_index: int):
    fn = DAO.functions.approveClientUpdate(Web3.to_checksum_address(job_addr), update_index)
    return _send(fn)


def rate_trainer(job_addr: str, score: int, comment: str):
    fn = DAO.functions.rateTrainer(Web3.to_checksum_address(job_addr), score, comment)
    return _send(fn)


def rate_requester(job_addr: str, score: int, comment: str):
    fn = DAO.functions.rateRequester(Web3.to_checksum_address(job_addr), score, comment)
    return _send(fn)
